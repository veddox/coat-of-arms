...
def create_menus():
    ...
    startm.entries = [("New Game", newgamem.execute),
                      ("Load Game", loadgamegm.execute),("Quit", quitm.execute)]
    newgamem.entries = [("Start game", startgamegm.execute),
                        ("Join game", joingamegm.execute),
                        ("Return to start menu", startm.execute)]
    quitm.entries = [("Yes", exit),("No", startm.execute),
                     ("Return to start menu", startm.execute)]

    startgamegm.add_entries(["Game name:", "Communication e-mail server:",
                             "E-mail account password:", createworldgm.execute,
                             createplayergm.execute])
    joingamegm.add_entries(["Game name:", "Communication e-mail server:",
                            "E-mail account password:", createplayergm.execute])
    loadgamegm.add_entries(["Absolute file path to the save file:"])
    createplayergm.add_entries(["Name:", "Nation:", "Age (Be careful!):"])
    #, (sexm = Menu([("Female", return "Female"), ("Male", return "Male")]))
    #.execute]) -- add this later?
